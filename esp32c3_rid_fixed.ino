/* -*- tab-width: 2; mode: c; -*-
 *
 * ESP32-C3 Remote ID Broadcast Example - FIXED VERSION
 *
 * This sketch broadcasts Remote ID signals using WiFi beacon mode
 * on an ESP32-C3 Mini with proper event loop initialization.
 */
...
// Add esp_event.h for proper event loop initialization
#include <esp_event.h>
...
void setup() {
  // Initialize the event loop FIRST - this fixes the error
  esp_err_t err = esp_event_loop_create_default();
  if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
    Serial.printf("Failed to create event loop: %d\n", err);
    return;
  }
  ...
  Serial.println("\n=== ESP32-C3 Remote ID Broadcast (FIXED) ===");
